...
pub fn run(input: &Value) -> Result<Value> {
    ...
    let rows = index.doc_links_for_symbol(symbol, exact)?;
    Ok(
        json!({"symbol": symbol, "exact": exact, "count": rows.len(), "docs": rows, "source": "repo_index_v2", "index_path": index_path.display().to_string()}),
    )
}
