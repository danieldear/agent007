...
pub async fn execute(_config: Arc<Config>, action: BundleAction) -> Result<()> {
    match action {
        BundleAction::Export { output, skills, workflows } => {
            ...
            println!("✓ Exported {} skill(s) and {} workflow(s) to {dest}",
                bundle.skills.len(), bundle.workflows.len());
            Ok(())
        }

        BundleAction::Import { file, overwrite, global } => {
            let content = std::fs::read_to_string(&file)
                .map_err(|e| anyhow::anyhow!("cannot read {file}: {e}"))?;
            let bundle = Bundle::from_json(&content)
                .map_err(|e| anyhow::anyhow!("invalid bundle: {e}"))?;

            println!("Bundle contains {} skill(s) and {} workflow(s)",
                bundle.skills.len(), bundle.workflows.len());

            let target = if global { agent007_global_home() } else { agent007_write_home() };
            ...
            let imported: Vec<_> = results.iter().filter(|r| r.action == agent007_sharing::ImportAction::Imported).collect();
            let skipped: Vec<_> = results.iter().filter(|r| r.action == agent007_sharing::ImportAction::Skipped).collect();
            let overwritten: Vec<_> = results.iter().filter(|r| r.action == agent007_sharing::ImportAction::Overwritten).collect();

            for r in &imported    { println!("  + {}", r.filename); }
            for r in &overwritten { println!("  ↺ {} (overwritten)", r.filename); }
            for r in &skipped     { println!("  ⊘ {} (skipped — use --overwrite)", r.filename); }

            println!("\n✓ Done: {} imported, {} skipped, {} overwritten",
                imported.len(), skipped.len(), overwritten.len());
            Ok(())
        }

        BundleAction::Promote { skill, workflow, global } => {
            let src_home = agent007_write_home();
            let target_home = if global { agent007_global_home() } else { agent007_global_home() };
...
fn promote_skill(src_home: &std::path::Path, target_home: &std::path::Path, trigger: &str) -> Result<()> {
    ...
    let found = std::fs::read_dir(&skills_src)?.flatten()
    ...
    if dest.exists() {
        anyhow::bail!("skill already exists globally at {}: use a different approach or remove it first", dest.display());
    }
...
fn promote_workflow(src_home: &std::path::Path, target_home: &std::path::Path, name: &str) -> Result<()> {
    let src_dir = src_home.join("workflows");
    let dest_dir = target_home.join("workflows");

    let src = [src_dir.join(format!("{name}.yaml")), src_dir.join(format!("{name}.yml"))]
        .into_iter()
        .find(|p| p.exists())
